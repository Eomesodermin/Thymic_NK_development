...
4. [The two evidence streams and the pivot between them](#4-the-two-evidence-streams-and-the-pivot-between-them)
...
The idea was that a subset of NK cells — enriched in CD56^bright — might be developmentally distinct, arising via a **thymic / aborted‑T‑cell pathway** and carrying a molecular relic of that history as **non‑productive / partial TCR rearrangements**. The first approach was to look for that footprint in single‑cell TCR (VDJ) data. It became clear this is **structurally the wrong assay**: a non‑productive rearrangement is a nonsense‑mediated‑decay (NMD) target, so it is silenced or rapidly degraded and barely transcribed — single‑cell mRNA‑based VDJ measures a deflated floor indistinguishable from ambient background, not the true rate. The footprint lives in genomic DNA, where this assay cannot see it. So the investigation **pivoted to somatic‑mtDNA lineage tracing (mtSNP)**, which reads clonal relationships directly from the genome and does not depend on transcription. But the mtSNP datasets are **sparse** — an exhaustive GEO survey confirmed no thymic deep‑mtDNA dataset exists at all, and only three usable NK‑bearing datasets are in hand. From the one decent tumor dataset there **appears to be a clonal‑restriction signal, but only in one tumor type (high‑grade serous ovarian) and only in ~2 patients** — too few to say anything confidently. The one place the data spoke with any strength is that CD56^bright and CD56^dim NK are **not cleanly two separate lineages** — but even that rests on very few informative clones (see §7.1), so it is better read as "no evidence for two lineages" than as a firmly established single lineage.
...
| **Q2′** | Are CD56^bright and CD56^dim NK one lineage or two? | mtDNA | **Not cleanly two lineages** — bright and dim share somatic‑mtDNA clones as a single mixed lineage predicts; clean segregation is excluded, but the test rests on few informative clones (38 in the marrow arm) | moderate (strongest in the project, but power‑limited) |
| **Q4** | Are tumor‑infiltrating NK cells clonally restricted? | mtDNA | **Apparent restriction in high‑grade serous ovarian only** (not lung) — but the signal rests on ~2 ovarian patients, so tumor‑type‑dependence is a hypothesis, not an established result | low–moderate |
...
**The honest bottom line:** the original hypothesis was not confirmed, and the project's main contribution is a clear‑eyed account of *why the available tools and data cannot yet settle it*. The TCR‑footprint approach is defeated by measurement biology (NMD silences the very transcripts that carry the signal); the mtSNP alternative is defeated by data scarcity (no thymic dataset, thin tumor cohorts). The two findings that do emerge — that bright/dim NK are *not cleanly two lineages*, and an *apparent* ovarian‑specific tumor clonal restriction — are the strongest the data support, but both are power‑limited (few informative clones; ~2 patients) and are stated here as constrained conclusions, not confident ones. This is a useful negative: the intuition was reasonable, and the project now maps precisely what would be needed to test it properly (§10).
...
## 4. The two evidence streams and the pivot between them

The project used two methodologically **orthogonal** lineage‑tracing readouts, but not with equal weight over time. It **began with the TCR‑footprint approach** (Stream A) — the most direct test of the "relic of a T‑like developmental state" idea. When it became clear that single‑cell mRNA‑based VDJ is *structurally* unable to measure that footprint (the non‑productive transcripts that carry it are NMD‑silenced; see §6.1), the primary effort **pivoted to somatic‑mtDNA lineage tracing** (Stream B), which reads clonal relationships from the genome without depending on transcription. The two readouts are genuinely complementary — each can test things the other cannot — which is why Stream A was not abandoned outright but rather demoted once its ceiling was understood:
...
> **How few cells this rests on — the load‑bearing caveat.** The headline NK counts (29,536 Rückert + 4,189 ReDeeM) are *not* the power basis: the sharing test only operates on **clones that contain testable NK of both subsets**, which is far smaller. The **ReDeeM marrow arm** — the progenitor‑proximal arm that would most cleanly separate two lineages — rests on **4 donors and just 38 testable clones total** (3, 7, 5, 23 per donor), and in **2 of those 4 donors both the observed and the null `frac_mixed` are exactly 1.0** (Old2: 3 clones; Young1.T2: 5 clones), meaning those donors carry essentially **no discriminating power** — every clone is trivially mixed. Only Young2 (23 clones) and Young1.T1 (7 clones) contribute any real spread. The **Rückert blood arm** has far more clones (867–1,631 per donor across 8 donors) but is ~90% CD56^dim, so the bright compartment is the binding limit (140–419 bright NK per donor vs thousands of dim). So the verdict is built on a **thin marrow arm plus a bright‑limited blood arm** — enough to exclude *clean* two‑lineage segregation, not enough to firmly establish a single lineage.
...
**Verdict — "not cleanly two lineages," and this is the strongest result in the project, but it is power‑limited.** Somatic mtDNA clones are **shared** between CD56^bright and CD56^dim NK, as a single mixed lineage predicts; a *clean* two‑lineage structure (which would drive `frac_mixed → 0`) is excluded. The result is directionally consistent across **blood *and* marrow**, two mtDNA chemistries (raw mtASAP + UMI‑consensus ReDeeM), and two label modalities (protein + RNA), and it is stable across every QC/caller setting swept — which is why it is the most robust thing the project produced. **But** (see the caveat box above) the discriminating marrow arm rests on only ~38 informative clones, with 2 of 4 donors contributing no power, so the honest reading is **"no evidence for two lineages"** rather than a firmly established single continuum. It leans against the project's two‑origin premise as originally framed, without settling it. Critically, it also does **not** exclude two transcriptionally distinct progenitor inputs (ENKP/ILCP) that converge clonally — that was not tested.
...
- **Q1 — footprint exists, but the assay can't confidently measure it (moderate):** NK cells do carry non‑productive TCR rearrangements at ~0.4–0.5% (3–18× below T‑cell classes), only marginally above the ambient/dropout floor. The deeper problem is that NMD silences these transcripts, so single‑cell mRNA VDJ measures a deflated floor — the assay is structurally unable to give a confident rate. Real, weak, and under‑measured.
...
- **Q2′ — one lineage vs two (strongest in project, but power‑limited):** bright and dim NK **share mtDNA clones as a single mixed lineage** — *clean* two‑lineage segregation is excluded, replicated across blood/marrow, two chemistries, two label modalities. This is the most robust result, but rests on few informative clones (38 in the marrow arm, 2/4 donors with no power), so read it as **"no evidence for two lineages"** rather than a firm single continuum. Leans against the two‑origin premise without settling it.
- **Q4 — apparent tumor restriction, ovarian only, thin (low–moderate):** an apparent seeding/restriction signal in HGSC **ovarian** and none in **lung** — but resting on ~2 ovarian patients. Consistent with (and sharpening) Liu, but tumor‑type‑dependence is a hypothesis at this n, not an established result.
...
**The overall picture.** The project set out to find evidence that CD56^bright NK are a developmentally distinct, thymic/aborted‑T‑derived subset marked by a TCR footprint. It could not confirm this — and, more usefully, it established *why*. The direct assay (single‑cell mRNA VDJ) is **structurally unable** to measure the footprint, because the non‑productive rearrangements that carry it are NMD‑silenced and barely transcribed. The pivot to mtSNP lineage tracing traded that measurement problem for a **data‑scarcity** problem: no thymic deep‑mtDNA dataset exists, and the usable tumor cohort is small. Within those limits, the two things the data did support both cut against or fail to support the original hypothesis: bright and dim NK show **no evidence of being two lineages** (the strongest but still power‑limited result), and the tumor clonal‑restriction signal is **apparent in ovarian only and rests on ~2 patients**. At the progenitor level the clonal‑tracking re‑analysis leans NK‑toward‑myeloid rather than NK‑toward‑T, again not supporting a shared T/NK origin — though the human literature is split.

This is an honest negative on the headline hypothesis, and its value is diagnostic: the original intuition was biologically reasonable, and the project now maps exactly where each approach breaks and what data would be needed to test it properly (§10). Two secondary contributions stand on their own regardless: the ovarian‑specific tumor‑NK restriction observation (which sharpens Liu, pending more patients) and the **methodological design rule** for mtDNA origin tests (permutation null *plus* a coverage‑matched cross‑lineage control).
...
- Software: cellranger 9.0.1, VDJ reference `refdata-cellranger-vdj-GRCh38-alts-ensembl-7.1.0`, `sc-dandelion_latest.sif` (dandelion 1.0.0a1.dev58). Reference: `HPC_workflows/marvin_hpc_reference.md`.
